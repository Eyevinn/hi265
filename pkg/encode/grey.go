...
// EncodeGreyIDRSliceFromSPSPPS encodes a grey IDR slice compatible with external SPS/PPS.
// A "grey" frame has uniform mid-grey value on all planes (128 for 8-bit, 512 for 10-bit).
// DC prediction produces exact grey for every CU, so residual is always zero.
// This makes the encoding independent of chroma format and bit depth.
// Returns Annex-B framed IDR_W_RADL NALU.
func EncodeGreyIDRSliceFromSPSPPS(sps *hevc.SPS, pps *hevc.PPS) ([]byte, error) {
	if err := validateGreySPSPPS(sps, pps); err != nil {
		return nil, err
	}

	p := greyIDRSliceParams(sps, pps)
	var buf bytes.Buffer
	WriteNALU(&buf, naluIDRWRadl, encodeGreyIDRSlice(p))
	return buf.Bytes(), nil
}

// greySliceParams holds parameters for encoding a grey IDR slice.
type greySliceParams struct {
	width  int
	height int
	qp     int
...
func greyIDRSliceParams(sps *hevc.SPS, pps *hevc.PPS) greySliceParams {
...
func validateGreySPSPPS(_ *hevc.SPS, pps *hevc.PPS) error {
...
// encodeGreyIDRSlice generates the complete IDR slice RBSP for a grey frame.
func encodeGreyIDRSlice(p greySliceParams) []byte {
	w := NewBitWriter()
	...
	// === Slice data (CABAC) ===
	cabacBytes := encodeGreySliceData(p)
...
// encodeGreySliceData encodes CABAC slice data for a grey IDR frame.
// Every CU uses DC prediction (mode 1) with zero residual.
// For a uniform grey surface, DC prediction produces the exact value:
// - First CU: no neighbors → reference samples default to 1<<(bitDepth-1) = mid-grey
// - Subsequent CUs: neighbors are all mid-grey → DC pred = mid-grey
// Therefore cbf_luma = cbf_cb = cbf_cr = 0 for all CUs.
func encodeGreySliceData(p greySliceParams) []byte {
	...
	for ctuY := 0; ctuY < p.height; ctuY += p.ctuSize {
		for ctuX := 0; ctuX < p.width; ctuX += p.ctuSize {
			encodeGreyCUTree(enc, models, ctuX, ctuY, p.ctuSize, p.log2CtuSize, p)
...
// encodeGreyCUTree recursively processes the coding quadtree.
// Follows HEVC spec 7.3.8.4 coding_quadtree().
func encodeGreyCUTree(enc *cabac.Encoder, models []cabac.CtxState,
	x0, y0, cuSize, log2CuSize int, p greySliceParams) {
	...
	if log2CuSize == p.log2MinCbSize {
		...
		encodeGreyCU(enc, models, x0, y0, log2CuSize, p)
		return
	}
	...
	for _, off := range [][2]int{{0, 0}, {half, 0}, {0, half}, {half, half}} {
		...
		if cx < p.width && cy < p.height {
			encodeGreyCUTree(enc, models, cx, cy, half, log2CuSize-1, p)
		}
	}
}

// encodeGreyCU encodes a single CU at minCbSize for a grey frame.
// DC prediction mode (1), zero residual for luma and both chroma components.
func encodeGreyCU(enc *cabac.Encoder, models []cabac.CtxState,
	x0, y0, log2CuSize int, p greySliceParams) {
	...
	encodeGreyTransformTree(enc, models, log2CuSize, 0, true, true, p)
}

// encodeGreyTransformTree encodes the transform tree for a grey CU (all cbf=0).
...
func encodeGreyTransformTree(enc *cabac.Encoder, models []cabac.CtxState,
	log2TrafoSize, trafoDepth int, parentCbfCb, parentCbfCr bool, p greySliceParams) {
	...
	if split {
		// Recurse into 4 children (parent cbf is always 0 for grey)
		for range 4 {
			encodeGreyTransformTree(enc, models, log2TrafoSize-1, trafoDepth+1, false, false, p)
		}
		return
	}
