...
import "github.com/Eyevinn/hi265/internal/cabac"
...
const (
	...
	// Total context count
	NumContextModels = 165
)

// CNU is "Context Not Used" — default initValue for unused contexts.
const cnu = 154

// lastSigCoeffInitValues has 30 values: 15 luma + 15 chroma.
// Used for both X and Y prefix (same init, separate state).
// From HM INIT_LAST[I_SLICE].
var lastSigCoeffInitValues = [30]uint8{
	// luma (15)
...
func init() {
	...
	copy(initValuesISlice[idx:], lastSigCoeffInitValues[:])
...
// InitModels initializes CABAC context models for the given slice QP.
// For Phase 1, only I-slice (sliceType=2) is supported.
func InitModels(sliceQPY int) []cabac.CtxState {
	models := make([]cabac.CtxState, NumContextModels)
	qp := clip3(0, 51, sliceQPY)
	for i := range NumContextModels {
		initValue := int(initValuesISlice[i])
