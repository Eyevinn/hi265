# Simplest possible general gray IDR frame generator

I want to make a new command hi265gray that constructs a gray IDR frame given VPS, SPS, PPS.
...
It is fine to only make this monochronic gray image with values
...
by not having a reconstruction image for prediction (provided that the prediction is also completely gray.

From a coding point of view, it is not necessary to have optimal
compression, but a simple choic of prediction is fine.

The present hi265gen can already make a gray image.
...
10-bit case. For a uniform surface, loop-filter should not
be needed either.
...
You may use ffmpeg to debug the generated gray image.
